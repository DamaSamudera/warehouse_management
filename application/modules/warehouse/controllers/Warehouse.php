...
class Warehouse extends MY_Controller
{
    ...
    {
        parent::__construct();
        $this->load->model('m_warehouse', 'main_model');
    }
    ...
    public function action_form($id = NULL){
        if($id){
            $this->data['warehouse'] = $this->main_model->get($id);
        }else{
            $this->data['warehouse'] = $this->main_model->get_new_data();
        }
    ...
    {
        $data_post = $this->main_model->array_from_post(array('code','name','email','phone','fax','mata_uang','area','alamat','is_active','is_warehouse','show_other_stock'));
        $data_post['is_active'] = ($data_post['is_active']) ? 'checked' : '';
        $data_post['is_warehouse'] = ($data_post['is_warehouse']) ? 'checked' : '';
        $data_post['show_other_stock'] = ($data_post['show_other_stock']) ? 'checked' : '';
