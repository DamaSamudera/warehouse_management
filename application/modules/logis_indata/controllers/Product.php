...
class Product extends MY_Controller
{
    ...
    public function action_form($id = NULL)
    {
        if ($id) {
            ...
            $category = $this->category_model->get($this->data['product']->product_id_category);
            $this->data['product']->product_id_category = $category->code . ' | ' . $category->name;
        ...
        #======================== TEMPLATE ==========================#
        $this->data['view_content'] = 'product/v_base_form';
        $this->data['script_content'] = 'product/v_script_form';
    ...
    {
        $data_post = $this->main_model->array_from_post(array('name', 'description', 'barcode', 'is_active', 'is_order', 'is_stock_tracking', 'n_supplier_id', 'product_id_category'));
        ...
        $data_post['product_id_category'] = $this->category_model->get_id($data_post['product_id_category']);
        ...
        $id_product = $this->main_model->save($data_post, NULL);
...
//            $id_inventory = $inventory->id;
        ...
        $id_inventory = $this->inventory_model->save($data_post_inventory, $id_inventory);
        ...
        $data_cart = array(
            ...
            "id_inv" => $id_inventory,
            "name" => $data_post['name'],
            "barcode" => $data_post['name'],
            "qty" => 1,
            "request_qty" => 0,
            "send_qty" => 0,
            "price" => $data_post_inventory['cost_distributor']
        );
        $this->cart->insert($data_cart);

        redirect('logis_indata/action_form', 'refresh');
    }
...
//            $this->main_model->delete($id);
    ...
    {
        header('Content-Type: application/json');
        $where = $this->input->post('category');
    ...
    public function get_list_category()
    {
        header('Content-Type: application/json');
        $where = $this->input->get('query');
        $result = $this->supplier_model->get_list_category($where);
        echo json_encode($result);
    }
}