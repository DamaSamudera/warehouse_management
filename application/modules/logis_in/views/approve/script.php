<script type="text/javascript">
    ...
    $(document).on('click', '.process', function(){
        ...
        $.ajax({
            url:"<?= base_url('logis_in/approve_proses'); ?>/" + "<?= $this->uri->segment(3); ?>",
            method:"POST",
            ...
            success:function(data)
            {
                //console.log(data);
                //location.reload();
                window.location.replace("<?= base_url('logis_in/surat_jalan')?>/" + data.id_stock_order + "/Input Order");
            }
        });
    ...
    function updatecart(component) {
        ...
        $.ajax({
            url: "<?php echo base_url('logis_in/approve_update_cart'); ?>",
            method: "POST",
